...
def output_word_topic_dist(llda, name, op_i, voca):
    phi = llda.phi()
    labs = []
    ...
    phi_df = pd.DataFrame(phi, index=voca, columns=labs)
    phi_df.to_csv("llda/llda_wordDistForDoc_{}_{}")
...
def llda_learning(llda, name, iteration, voca, anat, op, op_i, tol=0.01):
    ...
    for i in range(iteration):
        ...
        if pre_perp != None:
            ...
            break
    output_word_topic_dist(llda, name, op_i, voca)
...
# run_llda("llda_abstracts", 475)
run_llda("llda_abstracts", 0)
